...
> **Project status:** Backend MVP in progress (FastAPI + Supabase). Frontend auth wiring, G-code upload, the print queue and job history live under `frontend/UWA-3D-Print-Farm/`. Printers are driven through PrusaLink and currently tested against the mock printers in `mockserver/`.

---

## Getting started (local)

Docker Compose runs the FastAPI API, Vinext/Vite UI, and mock PrusaLink printers against the team Supabase project.
...
### Windows users: backend does not start

Windows Git settings can convert `backend/docker-entrypoint.sh` from Unix
`LF` to `CRLF` line endings. If the backend logs contain either
`exec ./docker-entrypoint.sh: no such file or directory` or
`set: Illegal option`, rebuild the backend image so the Dockerfile normalizes
the script before starting it:
...
Check that the backend is available before opening the UI:

```powershell
Invoke-WebRequest http://localhost:8000/health
```

The response should include `"status": "ok"`. Do not change the Supabase
database settings for this error; it occurs before the API starts.

Compose loads `backend/.env`, builds three images, and starts:

| Service | Container | Port | Build context |
|---|---|---|---|
| `mockserver` | `mockserver` | 8080 | `./mockserver` |
...
| http://localhost:8080/monitor | Mock printer live monitor |
| http://localhost:8080/docs | Mock printer operator API |

`frontend` depends on `backend`; `backend` depends on `mockserver` and reaches it at `http://mockserver:8080` (`MOCK_PRINTER_BASE_URL`). The browser calls the API at `http://localhost:8000` (`NEXT_PUBLIC_API_BASE_URL` in `docker-compose.yml`). Uploads persist in `./backend/storage` (mounted into the `backend` container; `FILE_STORAGE_ROOT` / `MAX_UPLOAD_BYTES` are environment-driven). Authenticated submitters can `POST /api/jobs/upload` (multipart `.gcode` / `.gco`) to create a `pending_selection` Print Job; unauthenticated, wrong-extension, oversize, and write/DB failures reject without lasting jobs or orphan files. Postgres and Auth come from Supabase (no local database container). Migrations are **not** applied on boot (`RUN_MIGRATIONS=0` against the shared DB).
...
### Demo with mock printers (no Supabase)

To see the whole flow (upload, validate, queue, print, history) without Supabase or a real printer, layer the demo file on top of the normal Compose file. From the **repository root**:

```bash
cp backend/.env.example backend/.env   # only if you have not already
docker compose -f docker-compose.yml -f docker-compose.demo.yml up --build
```

This starts the same `backend` and `frontend`, plus:

| Service | Container | Port | Build context |
|---|---|---|---|
| `db` | auto-named (Postgres 16, demo data only) | not published | image `postgres:16` |
| `mockserver` | auto-named (mock Prusa printers) | 8080 | `./mockserver` |
...
| http://localhost:8000/docs | Swagger UI |
| http://localhost:8080/docs | Mock printer operator API |
| http://localhost:8080/prusalink/mock-coreone-01/api/v1/status | Mock CORE One, PrusaLink API (Digest auth: `maker` / `mock-core-token`) |
| http://localhost:8080/prusalink/mock-xl-01/api/v1/status | Mock XL, PrusaLink API (Digest auth: `maker` / `mock-xl-token`) |

The demo file overrides the backend to use `AUTH_ADAPTER=fake` (accounts are seeded on each start), applies migrations and seeds two printers and three materials on boot (`RUN_MIGRATIONS=1`, `RUN_SEED=1`), and turns on the printer poller (`PRINTER_POLL_INTERVAL_S=2`). Do not use it against the shared Supabase database.

**Log in.** With the fake Auth adapter no email is sent, so the demo creates its accounts itself on every backend start (`DEMO_ACCOUNTS` in `docker-compose.demo.yml`):

| Role | Card | Email | Password |
|---|---|---|---|
...
To use your own email, put `DEMO_ACCOUNTS=email:password:role,...` in a `.env` file at the repository root (roles: `student`, `farmer`, `admin`; no commas or colons in passwords) and run `docker compose -f docker-compose.yml -f docker-compose.demo.yml up -d backend`. Existing profiles and job history are reused.

Then sign in, open **Upload file**, choose `raw/Rook1_0.4n_0.15mm_PLA_COREONE_1h5m.gcode`, pick **PLA White** and submit. The job starts on the mock CORE One within a few seconds. The mock runs 60x faster than real time, so this 1 h 5 m print finishes in about a minute. Then check **My jobs**.

The mock printers need Docker on macOS: the Prusa SDK they are built on relies on Linux `inotify`. Its logs repeat `host.docker.internal:9000 ... Connection refused`; that is the mock's legacy Connect transport and is harmless. Details: [`mockserver/README.md`](mockserver/README.md).

**Port conflicts.** If a previous stack is still running, stop it first: `docker compose down`, and `docker rm -f prusa-mock` if you started the mock by hand. Stop the demo and delete its database with `docker compose -f docker-compose.yml -f docker-compose.demo.yml down -v`.
...
**Ownership:** that retention/delete rule belongs to the **job lifecycle / collection** slice — not to `POST /api/jobs/upload`. Upload only cleans up on its own gate/write/commit (and future #3 validation) failures.
...
- Email/password Auth with Student Sign-up (email confirmation), Sign-in, `/me`, and RBAC probes
- G-code upload and validation (`backend/app/validation`), compatible-printer selection, job submission
- Per-printer print queue with estimated start and completion times; print history and farm statistics
- PrusaLink printer adapter and background sync (send queued jobs, track status, notification rows), tested against the mock printers
- Frontend: live dashboard, upload flow, and queue / job history

### Not Yet Finalised / not built

- Frontend: Notifications page, Farmer screens (collection, maintenance) and Admin screens (users, pricing, materials)
- PrusaLink against a real printer (only tested with the mock; auth and firmware behaviour unconfirmed)
- UWA-approved G-code validation profiles and policy values
- Pricing values
...
## Automated queue demo

To submit a job every three minutes from rotating department test accounts, see
[the queue demo guide](Docs/queue-demo.md). It runs against the local mock printer
and shows the waiting jobs in the Shared queue.
